 ...
 */
public class VolleyApptListSession  {
    ...
    int clinicid = 0;String datestring,appt_mode ="";
    ...
    public VolleyApptListSession(Context _mcontext) {
        _vcontext = _mcontext;
    ...
    //===========================================================
    public void GetApptListSessionJson( int clinicid,String appt_mode, String datestring, final String accesstoken, VolleyCallback callback) {
        showpDialog();
        this.clinicid = clinicid;
        this.datestring = datestring;
        this.appt_mode = appt_mode;
        String uri = String
                .format(Constant.URL_LISTAppointmentSlot + clinicid + "/" + datestring + "/" + "MobileApptView");
        Log.e("apptslot request",uri);
    ...
    public void setTvResultText(JSONObject response, VolleyCallback callback) {
        ...
        try {
            ...
            if (status.equals(Constant.CHECK_STATUS_OK)) {
                ...
                anydoctor_availabelsession.clear();
                anydoctor_availabletime.clear();
    ...
    public AppointmentSlot GetListApptSlot(JSONObject _data) {
        ...
        List<String> available_session = new ArrayList<String>();
        try {
            if (_data.has(Constant.NODE_ApptDoctor)) {
                ...
                if (appt_json.has(Constant.NODE_ApptStatus)) {
                    new_listappt.setStatus(appt_json.getInt(Constant.NODE_ApptStatus));
                }
            ...
            if(_data.has(Constant.NODE_ApptSession)){
                JSONArray availablesession = _data.getJSONArray(Constant.NODE_ApptSession);
                for (int i = 0; i < availablesession.length(); i++) {
                    JSONObject sessionObj = availablesession.getJSONObject(i);
                    String session = sessionObj.getString("Session");
                    String sessionTime = sessionObj.getString("SessionTime");

                    String add_session = sessionTime ;
                    available_session.add(add_session);
                    if(!anydoctor_availabelsession.contains(add_session)){
                        anydoctor_availabelsession.add(add_session);
                    }
                }
                new_listappt.setApptSession(available_session);
            }
        } catch (Exception ex) {
    ...
    public void getError(String error, VolleyApptListSession.VolleyCallback callback) {
        hidepDialog();
        try {
