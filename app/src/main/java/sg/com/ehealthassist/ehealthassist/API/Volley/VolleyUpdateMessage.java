...
public class VolleyUpdateMessage {
    private String Tag_json_obj = "_req_updatemessages";
    Context _vcontext;

    public interface VolleyCallback {
        void onSuccess(String messag);

        void onFail(String errorcode, String errormessage);
    }
    public VolleyUpdateMessage(Context context) {
        _vcontext = context;
    }
    public void UpdateMessageRequest(JSONObject param,final String accesstoken, VolleyCallback callback) {

        JsonObjectRequest myReq = new JsonObjectRequest(Request.Method.POST,
                Constant.URL_UpdateMessage,param,
           ...
           if(resultjson.has("Status")){
               ...
               if(status.equals("Ok")){
                   if(resultjson.has(Constant.NODE_DATA)){
                       JSONObject data = resultjson.getJSONObject(Constant.NODE_DATA);
