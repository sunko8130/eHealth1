 ...
 */
public class VolleyApptConfirm {

    private String Tag_json_obj = "_req_appt_confirm";
    Context _vcontext;
    private ProgressDialog pDialog;

    public VolleyApptConfirm(Context _mcontext) {
        _vcontext = _mcontext;
    ...
    public void GetApptConfirmJson(final String accesstoken, JSONObject param, VolleyCallback callback) {
        showpDialog();
        Log.e("rqconfirmjson", param.toString());

        String uri = String
                .format(Constant.URL_APPTCONFIRM);
            ...
            if (status.equals(Constant.CHECK_STATUS_OK)) {
                ...
                if (data.has(Constant.NODE_IsSuccess)) {
                    if (data.getBoolean(Constant.NODE_IsSuccess)) {
                        if (data.has(Constant.NODE_MESSAGE)) {
                            callback.onSuccess(data.getString(Constant.NODE_MESSAGE));
                        }
                    }
                }
            }
            Log.e("apptconfirm", response.toString());
